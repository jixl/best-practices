...
1. 创建 Pull Request 前，被合并的分支需要提前解决代码冲突
2. 解决完冲突后，创建合并请求
3. 示例：分支 `hotfix/testAAA` 合并到 `master` 主干分支上

```
1. git pull origin master # 如有冲突，解决冲突部分，若未按配置项 git config --global pull.rebase true 进行配置，则需要加参数 --rebase=true,即为 git pull origin master --rebase=true
2. git push (git push origin HEAD:hotfix/testAAA) -f
3. 创建pull request 请求
    base fork: master
    head fork: hotfix/testAAA
4. 合并指派人审查代码、完成合并请求
```
